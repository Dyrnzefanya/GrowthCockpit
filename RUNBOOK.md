...
For Meta and HubSpot, prefer **Settings → Integrations** when the production workspace has been activated and migration 0033+ is present. Enter the replacement value, save, and run **Test Connection** before revoking the old provider credential. The old value is never returned. If the card says **Server Environment**, rotate the named Vercel value and redeploy instead. Removing a Settings-managed value may reactivate that environment fallback, as shown before removal.
...
| `HUBSPOT_ACCESS_TOKEN`       | Create a least-privilege private-app token for the production portal. Replace it through Integration Center, or replace the Vercel fallback and redeploy. Test before revoking the former token, then run reconcile.                 | Integration Center verifies the portal; HubSpot card and `JOB-HUBSPOT-RECONCILE` show a new success.          |
| `HUBSPOT_WEBHOOK_SECRET`     | Rotate the app secret in HubSpot and replace it through Integration Center as the same maintenance change, or replace the Vercel fallback and redeploy. Update the production webhook subscription if HubSpot requires it.           | A real signed webhook is accepted and a tampered signature remains 401.                                       |
...
| `META_ACCESS_TOKEN`          | Issue a production System User token with `ads_read` only. Replace it through Integration Center, or replace the Vercel fallback and redeploy. Test before revoking the former token.                                                | Integration Center verifies account/scope; run `JOB-META-INGEST` and match currency/timezone metadata.        |
