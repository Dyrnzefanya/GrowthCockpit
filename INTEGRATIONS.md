...
Authorized owners manage implemented provider credentials at `/settings/integrations`. The typed registry currently exposes Meta Ads and HubSpot as functional providers. GA4, Google Search Console, and Google Ads are **COMING SOON / NOT IMPLEMENTED**: they accept no credentials and offer no Test Connection or Sync action. Slack and landing ingestion remain on their established environment and operational paths.

Secrets are write-only and encrypted by Supabase Vault. The browser receives only configuration source, configured flags, timestamps, safe provider identity, and sanitized error codes. Non-secret configuration is Meta Ad Account ID/API version and HubSpot Portal ID. Secret configuration is Meta Access Token, HubSpot Private App Access Token, and HubSpot Webhook Signing Secret. Bootstrap values such as Supabase keys, machine secrets, URL/timezone, job controls, and optional provider environment fallbacks remain server environment configuration.

Resolution precedence is a complete Settings-managed configuration, then a complete server environment configuration, then Not Configured. No environment secret is copied into Vault. Removing a Settings-managed credential deletes its Vault values; the UI warns when an environment fallback will become active. The settings page never retrieves a saved secret and has no reveal action.

Meta Test Connection calls only account and token-inspection GET endpoints, verifies the configured account, `ads_read`, absence of `ads_management`, currency, timezone, and available expiry metadata. HubSpot Test Connection calls account and property-schema reads to verify portal identity and the scopes required by the current adapter; it creates or modifies no CRM record. Both paths persist only allowlisted identity metadata and safe audit/error codes. Ingest, reconciliation, webhook verification, Meta read-only policy, HubSpot field ownership, and manual qualification-override behavior remain unchanged.

For an isolated personal development trial, keep the application pointed at the local/development Supabase project, open `/settings/integrations`, save personal provider configuration, and run Test Connection. Use `/integrations` for Meta ingest and `/integrations/hubspot` for the existing mapping/reconciliation controls. Confirm imported records only in development. This evidence is development UAT and does not satisfy any Du Anyam production-activation requirement.

To implement a future provider, first change its registry entry from `coming_soon` to `available` and declare only the fields and capabilities the real adapter needs. Add its Zod boundary schema, Vault allowlist/migration, centralized resolver mapping, provider adapter, read-only connection test where applicable, and focused authorization/leakage/provider tests. The generic card and state model need no redesign. Do not change availability until the provider calls and tests exist.
...
- Configuration: `/settings/integrations` stores the numeric provider account ID without `act_`, pinned API version, and write-only `ads_read` token through Supabase Vault. Existing server-only `META_AD_ACCOUNT_ID`, `META_ACCESS_TOKEN`, and optional `META_API_VERSION=v26.0` remain fallback values. Missing configuration displays an honest disconnected state. Never place tokens in control documents.
...
- `/settings/integrations` stores `HUBSPOT_ACCESS_TOKEN` and `HUBSPOT_WEBHOOK_SECRET` as write-only Vault secrets and numeric `HUBSPOT_PORTAL_ID` as non-secret configuration. The same server environment names remain fallback values. `APP_BASE_URL` supplies the trusted canonical signature URL. Absent credentials are Not Configured. No real HubSpot values are seeded.
