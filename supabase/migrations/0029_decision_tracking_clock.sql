-- Phase 11: include today in tracking facts; fence material snooze overrides.
create or replace function public.decision_facts(p_from date,p_to date) returns jsonb
...
begin
 select jsonb_build_object(
  ...
  'leads',coalesce((select jsonb_agg(to_jsonb(l)) from (
   ...
   from public.leads where inquiry_date between p_from and p_to+1 and inquiry_at<=now() group by platform,campaign_id,lt_campaign,inquiry_date
   order by inquiry_date,platform,campaign_id,lt_campaign limit 10001
  ) l),'[]'::jsonb),
  'followups',coalesce((select jsonb_agg(to_jsonb(l)) from (
   select l.id,l.qualification_status,d.amount::text as pipeline_value,d.currency as pipeline_currency,d.stage_category,coalesce((select max(changed_at) from public.lead_stage_events e where e.lead_id=l.id),l.inquiry_at) last_activity
   from public.leads l left join public.deals d on d.id=l.deal_id where qualification_status in ('mql','sql') order by l.id limit 10001
  ) l),'[]'::jsonb),
...
create or replace function public.commit_decisions(p_run uuid,p_rows jsonb,p_cursor text) returns integer
...
begin
 ...
 for item in select value from jsonb_array_elements(p_rows) loop
  ...
  if item->'alert' is not null and item->'alert'<>'null'::jsonb then
   ...
   if (item->>'override_snooze')::boolean and (prior.id is null or prior.updated_at is distinct from (item->>'alert_revision')::timestamptz) then raise exception 'ACTION_REVISION_CHANGED' using errcode='40001'; end if;
   if (item->>'override_snooze')::boolean and prior.id is not null and prior.updated_at=(item->>'alert_revision')::timestamptz then
    update public.alerts set snooze_until=null,status='open',suppressed_until=null,
      history=history||jsonb_build_array(jsonb_build_object('event','snooze_overridden','at',item->>'evaluated_at','reason','severity_or_evidence_worsened')) where id=prior.id;
   end if;
